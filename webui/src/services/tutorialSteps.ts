...
const catalog: StepCopy[] = [
  ...
  { id: 'prepare', chapter: 1, target: 'controller-status', zh: ['开始前的准备', '真实检测时请启动游戏并安装 ViGEmBus。检测期间让游戏保持前台聚焦，不要连接实体手柄。并在游戏中将摇杆灵敏度降到非常低'], en: ['Pre-Measurement Preparation', 'For real measurements, please launch the game and install ViGEmBus. Keep the game focused in the foreground during testing, do not connect physical controllers, and set the in-game stick sensitivity to very low.'] },
  ...
  { id: 'delete-points', chapter: 6, page: 4, target: 'analysis-table', zh: ['手动删除异常点位', '若个别采样点因画面抖动或掉帧出现较大偏差，可在下方数据明细表中手动删除对应点位；系统将自动剔除并即时重新拟合曲线。'], en: ['Manually Delete Outlier Points', 'If individual sample points deviate significantly due to camera jitter or dropped frames, you can manually delete them in the data table below; the system will automatically exclude them and immediately refit the curve.'] },
  { id: 'export', chapter: 6, page: 4, target: 'analysis-export', zh: ['导出 ControllerMeta 曲线数据', '导出的 JSON 是 ControllerMeta 曲线工具支持的格式。ControllerMeta 是需要另行下载安装的手柄检测工具，可前往 https://www.controllermeta.com/ 下载并用它继续编辑、转换曲线。教程不会打开或下载文件。'], en: ['Export ControllerMeta Curve Data', 'The exported JSON uses a format supported by the ControllerMeta curve tool. ControllerMeta is a separate gamepad testing tool that must be downloaded and installed from https://www.controllermeta.com/ before you can use it to continue editing or converting the curve. The tutorial does not open or download files.'] },
  ...
  { id: 'feedback', chapter: 7, zh: ['测试说明与交流反馈', '此测试方法和工具还在测试开发中，精确程度受到多种因素影响。如有任何疑问欢迎加群交流讨论：https://qm.qq.com/q/PSfZ4nbEUW'], en: ['Notice & Discussion Group', 'This testing methodology and tool are still in active development, and accuracy is influenced by various factors. If you have any questions, welcome to join our QQ discussion group: https://qm.qq.com/q/PSfZ4nbEUW'] },
]
